...
def _get_plugin(gl=False):
    ...
    try:
        torch.utils.cpp_extension._import_module_from_library(plugin_name, os.path.join(os.path.dirname(__file__), '..'), True)
        _cached_plugin[gl] = importlib.import_module('nvdiffrast.' + plugin_name)
        return _cached_plugin[gl]
    except:
        pass
    ...
    if gl:
        ...
        source_files = [
            ...
            '../common/cudaraster/impl/RasterImpl_cuda.cu',
            '../common/cudaraster/impl/RasterImpl.cpp',
            '../common/common.cpp',
            '../common/rasterize.cu',
            '../common/interpolate.cu',
            '../common/texture_cuda.cu',
            '../common/texture.cpp',
            '../common/antialias.cu',
            'torch_bindings.cpp',
            'torch_rasterize.cpp',
