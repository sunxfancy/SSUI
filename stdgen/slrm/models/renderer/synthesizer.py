...
from .utils.ray_sampler import OrthoRaySampler, RaySampler
...
class TriplaneSynthesizer(nn.Module):
    ...
    def __init__(self, triplane_dim: int, samples_per_ray: int, is_ortho: bool = False):
        super().__init__()
        ...
        # ray sampler
        if is_ortho:
            self.ray_sampler = OrthoRaySampler()
            self.is_ortho = True
        else:
    ...
    def forward(self, planes, cameras, render_size=128, crop_params=None, levels=None):
        # planes: (N, 3, D', H', W')
        # cameras: (N, M, D_cam)
        # render_size: int
        ...
        if self.is_ortho:
            ortho_scale = cameras[..., 25].view(N, M)
        else:
            ortho_scale = None

        self.rendering_kwargs['levels'] = levels.repeat_interleave(M, dim=0)
        ...
        rgb_samples, depth_samples, semantic_samples, weights_samples = self.renderer(
        ...
        semantic_samples = semantic_samples.permute(0, 2, 1).reshape(N, M, semantic_samples.shape[-1], Himg, Wimg)
        ...
        out = {
            ...
            'images_semantic': semantic_samples,
        }
        return out
