...
def get_center_boundary_index(grid_res_xy, grid_res_z, device):
    v = torch.zeros((grid_res_xy + 1, grid_res_xy + 1, grid_res_z + 1), dtype=torch.bool, device=device)
    v[grid_res_xy // 2 + 1, grid_res_xy // 2 + 1, grid_res_z // 2 + 1] = True
    center_indices = torch.nonzero(v.reshape(-1))

    v[grid_res_xy // 2 + 1, grid_res_xy // 2 + 1, grid_res_z // 2 + 1] = False
    v[:2, ...] = True
    v[-2:, ...] = True
    v[:, :2, ...] = True
    v[:, -2:, ...] = True
...
class FlexiCubesGeometry(Geometry):
    def __init__(
            self, grid_res_xy=64, grid_res_z=64,
            scale_xy=2.0, scale_z=2.0, device='cuda', renderer=None,
        ...
        self.verts, self.indices = self.fc.construct_voxel_grid([grid_res_xy, grid_res_xy, grid_res_z])
        ...
        self.verts[:, 0] = self.verts[:, 0] * scale_xy
        self.verts[:, 1] = self.verts[:, 1] * scale_xy
        self.verts[:, 2] = self.verts[:, 2] * scale_z
        ...
        self.center_indices, self.boundary_indices = get_center_boundary_index(self.grid_res_xy, self.grid_res_z, device)
    ...
    def get_mesh(self, v_deformed_nx3, sdf_n, weight_n=None, with_uv=False, indices=None, is_training=False):
        ...
        verts, faces, v_reg_loss = self.fc(v_deformed_nx3, sdf_n, indices, [self.grid_res_xy, self.grid_res_xy, self.grid_res_z],
    ...
    def render_mesh(self, mesh_v_nx3, mesh_f_fx3, camera_mv_bx4x4, resolution=256, hierarchical_mask=False, dtype=None):
        return_value = dict()
