...
class VenvManager:
    ...
    def create_for_module(self, name: str) -> VenvContainer:
        if name in self.venvs:
            return VenvContainer(self.get_base_path(), self.get_module_path(name), None)
        path = self.get_module_path(name)
        self.run_venv(path)
        self.create_pth(path, [path, self.get_base_path()])
        self.venvs[name] = path
        return VenvContainer(self.get_base_path(), path, None)

    def create_for_app(self, module_path: str, name: str) -> VenvContainer:
        if name in self.app_venvs:
            return VenvContainer(self.get_base_path(), module_path, self.app_venvs[name])
        path = self.get_app_path(name)
        self.run_venv(path)
        self.create_pth(path, [path, module_path, self.get_base_path()])
        self.app_venvs[name] = path
        return VenvContainer(self.get_base_path(), module_path, path)
