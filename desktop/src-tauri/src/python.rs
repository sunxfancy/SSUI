...
impl BackgroundProcessesGuard {
    ...
    pub fn add_process(&mut self, child: Child) {
        debug!("添加后台进程");
...
impl ProcessManager {
    ...
    pub fn add_process(&mut self, process_type: &str, child: Child) {
        debug!("添加进程: {}", process_type);
        self.processes.insert(process_type.to_string(), child);
    }

    pub fn is_process_running(&mut self, process_type: &str) -> bool {
...
pub struct GlobalProcessState {
    processes_guard: Mutex<BackgroundProcessesGuard>,
    process_manager: Mutex<ProcessManager>,
}

impl GlobalProcessState {
    fn new() -> Self {
        Self {
            processes_guard: Mutex::new(BackgroundProcessesGuard::new()),
            process_manager: Mutex::new(ProcessManager::new()),
        }
    }

    pub fn processes_guard(&self) -> MutexGuard<'_, BackgroundProcessesGuard> {
        self.processes_guard.lock().unwrap()
    }

    pub fn process_manager(&self) -> MutexGuard<'_, ProcessManager> {
        self.process_manager.lock().unwrap()
    }
}

pub static GLOBAL_PROCESS_STATE: LazyLock<GlobalProcessState> =
    LazyLock::new(GlobalProcessState::new);
...
pub async fn run_python_background(
    ...
    let log_file = File::create(&log_path).map_err(|e| format!("无法创建日志文件: {}", e))?;

    let error_log_file =
        File::create(&error_log_path).map_err(|e| format!("无法创建错误日志文件: {}", e))?;
    ...
    // 将子进程添加到全局列表中
    GLOBAL_PROCESS_STATE.processes_guard().add_process(child);
...
pub async fn get_server_status() -> Result<ProcessStatus, String> {
    ...
    Ok(ProcessStatus { is_running, pid })
}
